...
<% 
	...
	int count = dbPro.getArticleCount(id); //현재 로그인 된 아이디로 작성된 글의 갯수를 가져옵니다.
	if(count > 0){ //게시물이 존재할 경우 데이타빈 리스트의 정보들을 가져온다.
		articleList = dbPro.getArticles(count,id);
...
<%
	...
	if(count !=0 ){
%>
	<!-- 결과가 있을 경우 정보에 대한 글을 입력 -->
	<center><h2><%=(String)session.getAttribute("id")%>님의 포스트목록입니다.</h2></center>
<%
...
<td>
	<tr>
		<form method="post" action="/Cookie_Session/post/post_Modify_Form.jsp">
			<table class="type10">
				<thead>
					<tr>
						<th scope="cols">Title</th>
						<th scope="cols"><%=article.getTitle() %></th>
					</tr>
				</thead>
				<tbody>
					<tr>
			...
			<input type = "hidden" value=<%=article.getCategory()%> name="category"/>
					<tr>
						<input type = "submit" value="수정">
								...
								<form method="post" action="/Cookie_Session/post/post_Delete_Form.jsp">
								<input type = "hidden" value=<%=article.getNum()%> name="num"/>
								<input type = "hidden" value=<%=article.getId()%> name="id"/>
								<input type = "hidden" value=<%=article.getTitle()%> name="title"/>
								<input type = "hidden" value=<%=article.getContent()%> name="content"/>
